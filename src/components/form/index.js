...
const FormComponent = () => {
    ...
    const sendQuery = async () => {
     ...
     const handleScikit=async()=>{
      try {
        const response = await fetch("http://localhost:8000/predict_expense_regression/", {
           method: "POST",
           headers: {
             "Content-Type": "application/json",
           },
    ...
    <div className="chat-box">
      <div className="data-box">
        {resData ? (
          ...
          <Robot isAnimating={isAnimating} loading={loading} category={category} month={month} ani={ani}/>
        )}
      </div>
      <form className="form-box" onBlur={stopAnimation}>
        <span>Select future date, category and Model before proceeding* </span>
        <input
          type="month"
          name="month"
          value={month}
